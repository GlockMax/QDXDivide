...
Flags(dwFlags)
{
	...
	else if (Flags & CGridCellNumeric::Math)
	{
		cDecimal = _T('.');
		sDigits += cDecimal;
	}
...
void CInPlaceNumEdit::OnUpdate()
{
    if (!Updating)
    {
        ...
        if (Temp == str)
        {
            ...
            if (Ok && Flags & (CGridCellNumeric::Math | CGridCellNumeric::Real | CGridCellNumeric::Currency))
            {
...
{
    // Math, Integer, Real, and Currency are mutually exclusive flags,
    // But one of them has to set
    if ((dwFlags & TypeMask) != Integer &&
		(dwFlags & TypeMask) != Real &&
		(dwFlags & TypeMask) != Math &&
...
BOOL CGridCellNumeric::Format(CString &TheString)
{
    ...
    if (m_dwFlags & (Math | Real | Currency))
    {
    ...
    for(int i = str.GetLength() - 1; i >= 0; --i)
    {
		...
		LPCTSTR ptr = _tcschr(digits, str[i]);
        if (ptr == NULL)
            str.Delete(i);
        else
        {
