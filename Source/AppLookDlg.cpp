	...
	CDialog(CAppLookDlg::IDD, pParent), m_bStartup(bStartup)
{
	...
	CWinAppEx &theApp = (CWinAppEx &)(*AfxGetApp());

	m_nAppLook = theApp.GetInt(_T("AppLook"), 6);
	m_nStyle = theApp.GetInt(_T("AppStyle"), 0);
	m_bShowAtStartup = theApp.GetInt(_T("ShowAppLookAtStartup"), FALSE);
...
void CAppLookDlg::SetLook()
{
	...
	CMainFrame* pMainFrame = DYNAMIC_DOWNCAST(CMainFrame, GetParent());
	if (pMainFrame != NULL)
	{
