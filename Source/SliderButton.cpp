...
static const int CtrlHeight = 20;
static int nDefaultHeight = 21;
...
CSliderButton::CSliderButton() :
	m_wndSlider (*this)
{
	m_dwStyle = WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_BOTTOM | TBS_TRANSPARENTBKGND | TBS_AUTOTICKS;
...
CSliderButton::CSliderButton (UINT uiId,
	...
	m_wndSlider (*this)
{
	m_dwStyle = dwStyle | WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_BOTTOM | TBS_TRANSPARENTBKGND | TBS_AUTOTICKS;
...
void CSliderButton::OnMove ()
{
	...
	{
		int Ydispl = max(0, (m_rect.Height() - CtrlHeight) / 2 + 1);
		m_wndSlider.SetWindowPos (NULL,
			m_rect.left + 1, m_rect.top + Ydispl, m_rect.Width () - 4, CtrlHeight,
...
void CSliderButton::SetHeight(int Height)
{
...
int CCustomSliderCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
