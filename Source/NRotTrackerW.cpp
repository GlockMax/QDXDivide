...
BOpenGLWnd::NRotTracker::NRotTracker(BOpenGLWnd *pV, char axes, const BPoint &iRotBase, WORD LeftButton)
	: New0(0., 0., 0., 1.), New1(0., 0., 0., 1.)
{
	...
	{
	case 'A':
		pView->CursorID = IDC_BCUR_A;
		break;
	case 'X'://Rotate around X-Axes in object coord
		pView->CursorID = IDC_BCUR_X;
		break;
	case 'Y'://Rotate around Y-Axes in object coord
		pView->CursorID = IDC_BCUR_Y;
		break;
	case 'Z'://Rotate around Z-Axes in object coord
		pView->CursorID = IDC_BCUR_Z;
		break;
	case 'S'://Scale
		pView->CursorID = IDC_BCUR_S;
		break;
	case 'P'://Pan
		pView->CursorID = IDC_BCUR_P;
		break;
	case 'W'://Window
		pView->CursorID = IDC_BCUR_W;
		glAccum(GL_LOAD, 1.0);
...
BOpenGLWnd::NRotTracker::~NRotTracker()
{
	pView->CursorID = 0;
}

void BOpenGLWnd::NRotTracker::OnChangedRect(const CRect &rectOld)
{
	// Clear sc
	if (Axes == 'W')
	{
	...
	if (NewOne)
	{
		...
		else
		{
			...
			double wz;
			pView->GetMouseCoord(point, &wz, &bx, &by, &bz);
			if (wz == 0.)
			{// get base point from RotBase instead of picked point
				double h;
				RotBase.Get(bx, by, bz, h);
...
void BOpenGLWnd::NRotTracker::DrawRect()
{
	if (PictureChanged)
	{
		glAccum(GL_LOAD, 1.0);
...
void BOpenGLWnd::NRotTracker::DrawTrackerRect(LPCRECT lpRect, CWnd *pWndClipTo, CDC *pDC, CWnd *pWnd)
{
	// Do not draw 
	if (Axes == 'W')
	{
...
BOOL BOpenGLWnd::NRotTracker::TrackRubberBand(CWnd* pWnd, CPoint point, BOOL bAllowInvert)
{
...
BOOL BOpenGLWnd::NRotTracker::TrackHandle(int nHandle, CWnd* pWnd, CPoint point,
	CWnd* pWndClipTo)
{
	ASSERT(nHandle >= 0);
